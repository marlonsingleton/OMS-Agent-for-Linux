...
module Fluent
  class OMSSyslogInput < Input
    Plugin.register_input('oms_syslog', self)

    OMS_SYSLOG_REGEXP = /^\<([0-9]+)\>(.*)/
    ...
    def receive_data_with_format(data, addr)
      m = OMS_SYSLOG_REGEXP.match(data)
      unless m
    ...
    def parse_text(text, addr, pri = nil)
      @parser.parse(text) { |time, record|
        ...
        facility = OMS_FACILITY_MAP[pri >> 3]
        priority = OMS_PRIORITY_MAP[pri & 0b111]
